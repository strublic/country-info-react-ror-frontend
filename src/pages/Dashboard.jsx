...
console.log("HEEERR DASHBOARD")
console.log("STT", state)
console.log("kkk", type)
    const getCountries = async () => {
        
            console.log("LOCAL: ", JSON.parse(window.localStorage.getItem("auth")))
    ...
    return (countries && countries.length > 0 ?
        <>
            <div className="dashboard">
                <h1>All Countries</h1>
                <Link to="/dashboard/new"><button>New country</button></Link>
                <Routes>

                    <Route path="/dashboard/:action"  element={<Form/>}/>
                    {/* <Route path="/dashboard/:action" render={(rp) => <Form {...rp} getCountries={getCountries} />}/> */}
                </Routes>
                <ul>             
